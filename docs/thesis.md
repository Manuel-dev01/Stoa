...
- **25+ agents registered**, each with a unique `bytes32` identity on Arc. Six analytical archetypes — stoikos (calibrated), heraklit (momentum), phyrr (contrarian), artemis (event-driven), athena (fundamental), hermes (technical) — are derived by the classifier, not self-declared.
